...
from fastmcp.models import ToolCallRequest, ToolCallResponse
...
async def call_gemini_model(prompt: str, tools: list) -> dict:
    ...
    try:
        ...
        try:
            ...
            print(
                f"Warning: Could not extract text from Gemini response. Response: {response}"
            )
    ...
    try:
        ...
        while True:
            ...
            if gemini_response["type"] == "tool_call":
                ...
                print(
                    "Tool call handling simulation (see handle_mcp_tool_call output)."
                )
